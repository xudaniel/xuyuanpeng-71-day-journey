...
test('stage preview and apply reject overlaps, reordered stages, and impossible dates',()=>{
  const state=core.defaultState(),id='stage-06-shenzhen-long';
  for(const patch of [{end:'2026-10-25'},{start:'2026-09-12'},{start:'2026-12-01',end:'2026-12-02'},{end:'2026-09-31'}]){
  ...
  assert.throws(()=>core.impactOfStageChange(seed,state,id,{city:'Shenzhen'}),/重叠/);
  assert.deepEqual(state.history,[]);
});

test('all nested intervals and retained buffers are compared even after ignoring one risk',()=>{
  const state=core.defaultState();
  state.events=[['A','09:00','17:00'],['B','10:00','11:00'],['C','12:00','13:00'],['D','17:15','18:00']].map(([id,start,end])=>({id,start,end,date:'2026-09-20'}));
  const risks=core.detectConflicts(state);
...
test('travel endpoints support international, overnight, and explicit invalid arrivals',()=>{
  const leg={id:'flight',date:'2026-10-25',departureTime:'17:00',departureTimeZone:'Asia/Tokyo',arrivalDate:'2026-10-25',arrivalTime:'16:00',arrivalTimeZone:'America/Toronto'};
  ...
  const state={travel:[leg],events:[{id:'arrival-meeting',date:'2026-10-25',start:'15:00',end:'17:00',timeZone:'America/Toronto'}]};
  assert.equal(core.detectConflicts(state)[0].kind,'overlap');
  const overnight={...leg,date:'2026-10-24',departureTime:'23:00',arrivalDate:'2026-10-25',arrivalTime:'02:00',arrivalTimeZone:'Asia/Tokyo'};
  assert.equal(core.travelInterval(overnight).end-core.travelInterval(overnight).start,180);
  assert.throws(()=>core.travelInterval({...overnight,arrivalDate:'2026-10-24'}),/到达/);
  assert.equal(core.zonedEpochMinutes('2026-03-08','03:30','America/New_York'),Date.parse('2026-03-08T07:30Z')/60000);
  assert.throws(()=>core.zonedEpochMinutes('2026-03-08','02:30','America/New_York'),/不存在/);
});

test('datetime-local defaults preserve Shanghai wall time and local date boundaries',()=>{
  const before=process.env.TZ;
  try {process.env.TZ='Asia/Shanghai';assert.equal(core.localDateTimeValue(new Date('2026-09-14T18:45Z')),'2026-09-15T02:45');}
