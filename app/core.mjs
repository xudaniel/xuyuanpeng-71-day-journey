...
export function validateJourneyData(data) {
  const errors=[];
  if (!data || data.schemaVersion !== SCHEMA_VERSION) errors.push('Unsupported journey schema version.');
  ...
  for (const stage of data?.stages || []) {
    ...
    try {
      if (dateOnly(stage.end) < dateOnly(stage.start)) errors.push(`${stage.id || 'stage'} ends before it starts.`);
    } catch (error) { errors.push(error.message); }
  }
  const sorted=[...(data?.stages||[])].sort((a,b)=>a.start.localeCompare(b.start));
  for(let i=1;i<sorted.length;i++){
    const prevEnd=dateOnly(sorted[i-1].end), currentStart=dateOnly(sorted[i].start);
...
export function defaultState() {
  return {schemaVersion:SCHEMA_VERSION, revision:0, updatedAt:new Date().toISOString(), stageOverrides:{}, events:[], travel:[], people:[], organizations:[], actions:[], interactions:[], notes:[], history:[], riskOverrides:[]};
}

export function effectiveStages(seed, state) {
  return seed.stages.map(stage=>({...stage, ...(state.stageOverrides?.[stage.id]||{})})).sort((a,b)=>a.start.localeCompare(b.start));
}

export function currentStage(seed, state, date=todayInZone(seed.trip.timeZone)) {
  return effectiveStages(seed,state).find(stage=>stage.start<=date && date<=stage.end) || null;
}

export function dayNumber(seed, date=todayInZone(seed.trip.timeZone)) {
  const n=Math.floor((dateOnly(date)-dateOnly(seed.trip.start))/DAY_MS)+1;
...
export function detectConflicts(state, options={}) {
  ...
  for(const event of state.events||[]) {
    ...
    const start=zonedEpochMinutes(event.date,event.start,event.timeZone), end=zonedEpochMinutes(event.endDate||event.date,event.end||event.start,event.timeZone);
  ...
  for(const b of items){
    ...
    let required=(a.location&&b.location&&a.location!==b.location)?Math.max(defaultMeetingBuffer,60):defaultMeetingBuffer;
    if(b.type==='travel'){
      const mode=(b.source.mode||'').toLowerCase();
      required=/flight|航班|airport|机场/.test(mode+' '+b.title)?airportBuffer:/train|高铁|rail|车站/.test(mode+' '+b.title)?stationBuffer:defaultMeetingBuffer;
    }
    if(gap<required){
...
export function relationshipSummary(state, personId) {
  ...
  const interactions=(state.interactions||[]).filter(i=>i.personId===personId).sort((a,b)=>(b.at||'').localeCompare(a.at||''));
  const actions=(state.actions||[]).filter(a=>a.personId===personId && !['completed','canceled'].includes(a.status));
  const waiting=actions.filter(a=>a.status==='waiting');
  const owed=actions.filter(a=>a.status!=='waiting');
  return {person,lastInteraction:interactions[0]||null,nextTouch:person.nextTouch||null,openLoops:actions.length,waiting,owed,timeline:[...interactions].slice(0,20)};
}

export function findPossiblePeopleDuplicates(state, candidate) {
  const norm=s=>(s||'').toLowerCase().replace(/\s+/g,'').replace(/[·._-]/g,'');
  const names=[candidate.name,...(candidate.aliases||[])].filter(Boolean).map(norm);
  return (state.people||[]).filter(p=>{
    const existing=[p.name,...(p.aliases||[])].filter(Boolean).map(norm);
    return names.some(n=>existing.includes(n)) || (candidate.email&&p.email&&candidate.email.toLowerCase()===p.email.toLowerCase()) || (candidate.phone&&p.phone&&candidate.phone===p.phone);
  });
}
...
export function impactOfStageChange(seed,state,stageId,patch) {
  ...
  if(dateOnly(after.end)<dateOnly(after.start)) throw new Error('结束日期不能早于开始日期。');
...
export function applyStageChange(seed,state,stageId,patch,reason='') {
  ...
  next.stageOverrides={...(next.stageOverrides||{}),[stageId]:{...before,...patch}};
  next.revision=(next.revision||0)+1; next.updatedAt=new Date().toISOString();
  next.history=[...(next.history||[]),{id:uid('change'),type:'stage-change',stageId,before,after:next.stageOverrides[stageId],reason,at:next.updatedAt,impact}];
  return next;
}

export function undoLastChange(state) {
  const history=[...(state.history||[])];
  const last=[...history].reverse().find(h=>h.type==='stage-change'&&!h.undoneAt);
  if(!last) return state;
  const next=structuredClone(state);
  next.stageOverrides={...(next.stageOverrides||{})};
  if(Object.keys(last.before||{}).length) next.stageOverrides[last.stageId]=last.before; else delete next.stageOverrides[last.stageId];
  const target=next.history.find(h=>h.id===last.id); target.undoneAt=new Date().toISOString();
  next.revision=(next.revision||0)+1; next.updatedAt=target.undoneAt;
  return next;
}

export function countsForBackup(state){
  return {events:(state.events||[]).length,travel:(state.travel||[]).length,people:(state.people||[]).length,actions:(state.actions||[]).length,notes:(state.notes||[]).length,history:(state.history||[]).length};
}